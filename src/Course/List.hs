...
foldRight 
  :: (a -> b -> b) -- ^ replace cons with the first argument
  -> b  -- ^ replace Nil with the second argument
  -> List a -- ^ in this List
  -> b -- ^ return the new value
foldRight _ b Nil      = b
...
filter ::
  ...
  -> List a
filter pred (x:.xs) =
  let t = filter pred xs
  in bool id (x :.) (pred x) t
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 (_a:._b:._c:._d:._e:._tail) = True
lengthGT4 _ = False
